// Main app layout: connects webcam tracking, 3D scene, map UI, debug UI, and progress.
...
import { ThreeStreetScene } from './components/ThreeStreetScene'
...
function App() {
  // Map state: shared by the button, M key, hands-raised gesture, and map overlay.
  ...
  const [worldDebug, setWorldDebug] = useState({
    armTurnCooldownMs: 0,
  ...
  // Collection/progress state arrives from the 3D scene after pickup checks.
  ...
  // Tracking setup: webcam refs and live pose/motion state from MediaPipe.
  const {
    canvasRef,
    handleCameraError,
    handleCameraReady,
  ...
  useEffect(() => {
    try {
      ...
      // Debug persistence is optional; the panel still works if storage is unavailable.
    }
  }, [isDebugOpen])
  ...
  return (
    // Layout render: Three.js canvas plus overlay UI panels and controls.
    <main className="street-prototype" aria-label="Pastel street visual prototype">
      <section className="street-frame">
        <ThreeStreetScene
        ...
        {pickupDebug.nearbyPart !== 'none' && pickupDebug.gestureState !== 'collected' && (
        ...
        <div className="keyboard-hint" aria-live="polite">
          Use ↑ ← → to move. Press ↓ to bend and pick up.
        </div>
        ...
        {!isRunning && (
          <button type="button" className="camera-start" onClick={startCamera} disabled={isLoading}>
